...
# Getting bot’s card function 2 (if player wins the previous round)
def bot_call_2():
    global b_call_2
    global winner_new
...
#getting all the trump cards to a list- function
def all_trump_cards ():
    for i in player_hand:
        if any(trump_suit in s for s in i):
            all_trump.append(i)

#checking the user entered value is applicable
def card_validity ():
...
#Cheking the user,bot's cards shape in their hands and passing the winner according to that
def shape_check_123():
    global winner
    if len(b_call) == 2:
...
#if player wins the previus round this will be called to check the user entered value
def card_validity_2 ():
    while u_call not in player_hand:
...
#To check the random card is correct shape or not
...
def replay():
    global r_play
    print ("NOTE : Type 'yes' to play, 'no' to exit, '0' for instructions")
    print ()
    r_play = input("You wanna play ? : ")


replay()
if r_play == "n" or r_play == "N" or r_play == "no" or r_play == "NO" or r_play == "No" or r_play == "":
        print ("END OF THE GAME")
        sys.exit()   #<----exiting the game
elif r_play == "0":
    instructions()

while r_play == "y" or r_play == "Y" or r_play == "Yes" or r_play == "YES" or r_play == "yes":
    start_game()
    creating_deck()
    ...
    while trump not in str(numbers):
        ...
        while Trick <= 8:
            ...
            if player_score == 16:
                ...
                print ("You won in all the tricks")
